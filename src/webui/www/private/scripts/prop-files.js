...
window.qBittorrent.PropFiles ??= (() => {
    ...
    const setContentPropertyText = (files) => {
        let propElem = document.getElementById("propFilesLink");
        if (propElem !== null) {
            let propertyText = "QBT_TR(Content (%1/%2))QBT_TR[CONTEXT=PropertiesPanel]";
            let completedFiles = files.filter((file) => {
